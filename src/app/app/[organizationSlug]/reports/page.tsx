...
export default async function ReportsPage({ params }: { params: Promise<{ organizationSlug: string }> }) {
  ...
  if (!context.activeLocation) return <EmptyState title="No active location" description="Select a pantry location before opening reports." />;
  ...
  return <div className="grid gap-10">
    <PageHeader eyebrow="Operations" title={`Reports · ${context.activeLocation.name}`} description="Canonical operational reports use the same ledger, reservation, fulfillment, forecast, and message records as the rest of the application." actions={can(context.effectivePermissions, "report.design") ? <Link className="inline-flex min-h-11 items-center border border-[var(--ink)] px-4 text-sm font-semibold" href={`/app/${organizationSlug}/reports/designer`}>Report designer</Link> : null} />
