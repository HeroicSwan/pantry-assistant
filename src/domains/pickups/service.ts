...
const db = {
  async transaction<T>(run: (tx: Transaction) => Promise<T>) {
    try {
      ...
      if (cause?.message && SAFE_DATABASE_ERRORS.has(cause.message))
        throw new DomainError(cause.message);
      if (cause)
        throw Object.assign(new Error("DATABASE_OPERATION_FAILED"), {
          code: cause.code,
        });
      throw error;
    }
...
async function requireOrgWidePermission(
  tx: Transaction,
  actorId: string,
  organizationId: string,
  permission: string,
  ...
  if (
    !(await hasPermissionAnywhereInOrganization(
      tx,
      actorId,
      organizationId,
      permission,
...
async function requireLocationPermission(
  ...
  if (!(await hasLocationPermission(tx, actorId, locationId, permission)))
    throw new DomainError("FORBIDDEN");
}

async function requireHouseholdScopePermission(
  tx: Transaction,
  actorId: string,
  organizationId: string,
  ...
  if (!locationId) {
    await requireOrgWidePermission(tx, actorId, organizationId, permission);
    return;
  }
  const [location] = await tx
    ...
    .where(
      and(
        eq(pantryLocations.id, locationId),
        eq(pantryLocations.organizationId, organizationId),
      ),
    )
    .limit(1);
  if (!location) throw new DomainError("FORBIDDEN");
  await requireLocationPermission(tx, actorId, locationId, permission);
}
...
export async function createHousehold(
  actorId: string,
  organizationId: string,
  values: HouseholdInput,
  requestId: string,
) {
  return db.transaction(async (tx) => {
    await requireHouseholdScopePermission(
      tx,
      actorId,
      organizationId,
      values.defaultPantryLocationId,
      "household.create",
    );
    if (!validateHouseholdCounts(values))
      throw new DomainError("HOUSEHOLD_SIZE_INVALID");
    const [household] = await tx
      ...
      .returning();
    await writeAudit(tx, actorId, organizationId, {
...
export async function updateHousehold(
  ...
  return db.transaction(async (tx) => {
    ...
    if (previous.status === "merged")
      throw new DomainError("HOUSEHOLD_NOT_ELIGIBLE");
    await requireHouseholdScopePermission(
      tx,
      actorId,
      organizationId,
      previous.defaultPantryLocationId,
      "household.update",
    );
    const nextLocationId = values.defaultPantryLocationId || null;
    if (nextLocationId !== previous.defaultPantryLocationId)
      await requireHouseholdScopePermission(
        tx,
        actorId,
        organizationId,
    ...
    const [updated] = await tx
      ...
      .set({
        displayName: values.displayName,
        preferredLanguage:
          values.preferredLanguage || previous.preferredLanguage,
        householdSize: values.householdSize,
        adultCount: values.adultCount ?? null,
        childCount: values.childCount ?? null,
        seniorCount: values.seniorCount ?? null,
    ...
    await writeAudit(tx, actorId, organizationId, {
      ...
      previousValues: {
        ...
        householdSize: previous.householdSize,
      },
      newValues: {
...
export async function addHouseholdContact(
  ...
  householdId: string,
  values: {
    contactType: typeof householdContacts.$inferInsert.contactType;
    name: string;
  ...
  return db.transaction(async (tx) => {
    const [household] = await tx
      .select({
        ...
        defaultPantryLocationId: households.defaultPantryLocationId,
      })
      .from(households)
      .where(
        and(
    ...
    await requireHouseholdScopePermission(
      ...
      actorId,
      organizationId,
      household.defaultPantryLocationId,
      "household.view_contact",
    );
    await requireHouseholdScopePermission(
      tx,
      actorId,
    ...
    const [contact] = await tx
      ...
      .returning();
    await writeAudit(tx, actorId, organizationId, {
      action: "household.contact_added",
      entityType: "household_contact",
      entityId: contact.id,
      requestId,
      newValues: {
        householdId,
...
export async function addHouseholdPreference(
  ...
  householdId: string,
  values: {
    preferenceType: typeof householdPreferences.$inferInsert.preferenceType;
    valueCode: string;
    displayLabel: string;
  ...
  return db.transaction(async (tx) => {
    ...
    const [preference] = await tx
      ...
      .returning();
    await writeAudit(tx, actorId, organizationId, {
      action: "household.preference_added",
      entityType: "household_preference",
      entityId: preference.id,
      requestId,
      newValues: {
        householdId,
        preferenceType: preference.preferenceType,
        valueCode: preference.valueCode,
...
export async function recordSmsConsent(
  ...
  householdId: string,
  values: {
    householdContactId?: string | null;
    phoneNumber: string;
    status: typeof smsConsents.$inferInsert.status;
    consentSource: typeof smsConsents.$inferInsert.consentSource;
    notes?: string | null;
  },
  requestId: string,
) {
  return db.transaction(async (tx) => {
    ...
    const [consent] = await tx
      ...
      .values({
        organizationId,
        householdId,
        householdContactId: values.householdContactId || null,
        phoneNormalized,
...
export async function createPackageTemplate(
  ...
  return db.transaction(async (tx) => {
    await requireOrgWidePermission(
      tx,
      actorId,
      organizationId,
      "package.manage",
    ...
    const [template] = await tx
      ...
      .values({
        organizationId,
        pantryLocationId: values.pantryLocationId || null,
        name: values.name,
      ...
      .returning();
    await writeAudit(tx, actorId, organizationId, {
      action: "package.template_created",
      entityType: "pickup_package_template",
      entityId: template.id,
      requestId,
      newValues: { name: template.name, packageType: template.packageType },
    });
    return template;
  });
}
...
export async function addPackageTemplateLine(
  ...
  templateId: string,
  values: {
    lineType: "exact_item" | "category_choice" | "optional_item";
  ...
  return db.transaction(async (tx) => {
    ...
    const [template] = await tx
      .select({ id: pickupPackageTemplates.id })
      .from(pickupPackageTemplates)
      .where(
        and(
          eq(pickupPackageTemplates.id, templateId),
          eq(pickupPackageTemplates.organizationId, organizationId),
          sql`${pickupPackageTemplates.archivedAt} is null`,
        ),
      )
      .limit(1);
    ...
    const [line] = await tx
      ...
      .values({
        ...
        inventoryItemId:
          values.lineType === "category_choice"
            ? null
            : values.inventoryItemId || null,
        inventoryCategoryId:
          values.lineType === "category_choice"
            ? values.inventoryCategoryId || null
            : null,
      ...
      .returning();
    await writeAudit(tx, actorId, organizationId, {
      action: "package.template_line_added",
      entityType: "pickup_package_template_line",
      entityId: line.id,
      requestId,
      newValues: {
        templateId,
...
export async function addSizeRule(
  ...
  return db.transaction(async (tx) => {
    ...
    const existing = await tx
      .select({
        minimumHouseholdSize: householdSizePackageRules.minimumHouseholdSize,
        maximumHouseholdSize: householdSizePackageRules.maximumHouseholdSize,
      })
      .from(householdSizePackageRules)
      .where(
        and(
          eq(householdSizePackageRules.packageTemplateId, templateId),
          eq(householdSizePackageRules.organizationId, organizationId),
          sql`${householdSizePackageRules.archivedAt} is null`,
        ),
      );
    const overlap = existing.some((rule) => {
      ...
      return (
        values.minimumHouseholdSize <= existingMax &&
        rule.minimumHouseholdSize <= newMax
      );
    });
    ...
    const [rule] = await tx
      ...
      .values({
        organizationId,
        packageTemplateId: templateId,
        minimumHouseholdSize: values.minimumHouseholdSize,
        maximumHouseholdSize: values.maximumHouseholdSize,
        quantityMultiplier: values.quantityMultiplier,
        createdBy: actorId,
      })
      .returning();
    await writeAudit(tx, actorId, organizationId, {
      action: "package.size_rule_added",
      entityType: "household_size_package_rule",
      entityId: rule.id,
      requestId,
      newValues: {
        templateId,
        minimumHouseholdSize: rule.minimumHouseholdSize,
        maximumHouseholdSize: rule.maximumHouseholdSize,
...
async function appendStatusHistory(
  tx: Transaction,
  appointment: { id: string; organizationId: string; pantryLocationId: string },
...
export async function createAppointment(
  ...
  return db.transaction(async (tx) => {
    await requireLocationPermission(
      tx,
      actorId,
      values.pantryLocationId,
      "appointment.create",
    );
    if (values.scheduledStartAt >= values.scheduledEndAt)
      throw new DomainError("APPOINTMENT_TIME_INVALID");
    const [household] = await tx
      .select({
        id: households.id,
        status: households.status,
        householdSize: households.householdSize,
        preferredLanguage: households.preferredLanguage,
      })
      .from(households)
      .where(
        and(
          eq(households.id, values.householdId),
          eq(households.organizationId, organizationId),
        ),
      )
      .limit(1);
    ...
    const [appointment] = await tx
      ...
      .values({
        ...
        rescheduledFromAppointmentId:
          values.rescheduledFromAppointmentId || null,
        createdBy: actorId,
      })
      .returning();
    ...
    await writeAudit(tx, actorId, organizationId, {
      ...
      locationId: appointment.pantryLocationId,
      requestId,
      newValues: {
        appointmentNumber: appointment.appointmentNumber,
        householdId: appointment.householdId,
        type: appointment.appointmentType,
        startAt: appointment.scheduledStartAt.toISOString(),
      },
    });

    if (values.generateAllocation && appointment.packageTemplateId) {
      await generateAllocationInTx(
        tx,
        actorId,
        organizationId,
        appointment.id,
    ...
    let reservationResult: Awaited<
      ReturnType<typeof createReservationInTx>
    > | null = null;
    if (values.reserve && appointment.packageTemplateId) {
      reservationResult = await createReservationInTx(
        tx,
        actorId,
        organizationId,
        appointment.id,
...
async function generateAllocationInTx(
  tx: Transaction,
  actorId: string,
  organizationId: string,
  appointmentId: string,
  ...
  if (!appointment.packageTemplateId)
    throw new DomainError("PACKAGE_TEMPLATE_NOT_FOUND");
  const [template] = await tx
    .select()
    .from(pickupPackageTemplates)
    .where(
      and(
        eq(pickupPackageTemplates.id, appointment.packageTemplateId),
        eq(pickupPackageTemplates.organizationId, organizationId),
      ),
    )
    .limit(1);
  ...
  const rules = await tx
    ...
    .where(
      and(
        eq(householdSizePackageRules.packageTemplateId, template.id),
        sql`${householdSizePackageRules.archivedAt} is null`,
      ),
    );
  const rule = selectSizeRule(
    rules.map((candidate) => ({
      id: candidate.id,
      minimumHouseholdSize: candidate.minimumHouseholdSize,
      maximumHouseholdSize: candidate.maximumHouseholdSize,
      quantityMultiplier: candidate.quantityMultiplier,
    })),
  ...
  const [allocation] = await tx
    ...
    .values({
      ...
      templateSnapshot: {
        ...
        allowSubstitutions: template.allowSubstitutions,
  ...
  await writeAudit(tx, actorId, organizationId, {
    action: "appointment.allocation_generated",
    entityType: "appointment_allocation",
...
export async function generateAppointmentAllocation(
  actorId: string,
  organizationId: string,
  appointmentId: string,
  requestId: string,
  ...
  return db.transaction(async (tx) => {
    ...
    await requireLocationPermission(
      tx,
      actorId,
      appointment.pantryLocationId,
      "appointment.update",
    );
    return generateAllocationInTx(
      tx,
      actorId,
      organizationId,
      appointmentId,
...
async function lockAndComputeLotAvailability(
  tx: Transaction,
  organizationId: string,
  pantryLocationId: string,
  itemId: string,
...
async function resolveCategoryItem(
  tx: Transaction,
  organizationId: string,
  pantryLocationId: string,
  categoryId: string,
...
async function createReservationInTx(
  ...
  reservation: typeof inventoryReservations.$inferSelect | null;
  ...
  if (!["scheduled", "confirmed", "arrived"].includes(appointment.status))
    throw new DomainError("APPOINTMENT_INVALID_STATE");
  ...
  const [existingByKey] = await tx
    ...
    .where(
      and(
        ...
        eq(inventoryReservations.idempotencyKey, options.idempotencyKey),
      ),
    )
    .limit(1);
  ...
  const [activeExisting] = await tx
    ...
    .where(
      and(
        eq(inventoryReservations.appointmentId, appointmentId),
  ...
  const [allocation] = await tx
    ...
    .where(eq(appointmentAllocations.appointmentId, appointmentId))
    .limit(1);
  ...
  for (const line of allocationLines) {
    ...
    if (!itemId && line.inventoryCategoryId) {
      itemId = await resolveCategoryItem(
        tx,
        organizationId,
        appointment.pantryLocationId,
        line.inventoryCategoryId,
      );
    }
    const requested = asQ(line.requestedBaseQuantity);
    if (!itemId) {
      conflicts.push({
        inventoryItemId: line.inventoryCategoryId ?? "unknown",
        itemName: "No available item in category",
        requested,
        reservable: 0,
        shortage: requested,
        isRequired: line.isRequired,
    ...
    const lots = await lockAndComputeLotAvailability(
      tx,
      organizationId,
      appointment.pantryLocationId,
      itemId,
    ...
    const plan = planFefoAllocation(
      lots.map((lot) => ({
        ...
        availableQuantity: asQ(lot.available),
      })),
      requested,
    );
    if (plan.shortage > 0) {
      conflicts.push({
        ...
        itemName: item.name,
        requested,
        reservable: requested - plan.shortage,
  ...
  for (const plan of planned) {
    const reservedTotal = plan.allocations.reduce(
      (sum, allocationPlan) => sum + allocationPlan.quantity,
      0,
    );
    ...
    const [line] = await tx
      ...
      .values({
        ...
        organizationId,
        pantryLocationId: appointment.pantryLocationId,
        appointmentAllocationLineId: plan.allocationLine.inventoryItemId
          ? plan.allocationLine.id
          : null,
...
async function releaseReservationInTx(
  ...
  if (!["active", "partially_fulfilled"].includes(reservation.status))
    return reservation;

  const lines = await tx
    .select()
    .from(inventoryReservationLines)
    .where(eq(inventoryReservationLines.reservationId, reservationId));
  for (const line of lines) {
    const remaining = Math.max(
      asQ(line.reservedBaseQuantity) -
        asQ(line.fulfilledBaseQuantity) -
        asQ(line.releasedBaseQuantity),
      0,
    );
    if (remaining > 0) {
      await tx
        ...
        .set({
          releasedBaseQuantity: fmtQ(
            asQ(line.releasedBaseQuantity) + remaining,
          ),
        })
        .where(eq(inventoryReservationLines.id, line.id));
    }
    const lotAllocations = await tx
      .select()
      .from(inventoryReservationLotAllocations)
      .where(
        and(
          eq(inventoryReservationLotAllocations.reservationLineId, line.id),
          eq(inventoryReservationLotAllocations.status, "active"),
        ),
      );
    for (const lotAllocation of lotAllocations) {
      const lotRemaining = Math.max(
        asQ(lotAllocation.reservedBaseQuantity) -
          asQ(lotAllocation.fulfilledBaseQuantity) -
          asQ(lotAllocation.releasedBaseQuantity),
        0,
      );
      await tx
        ...
        .set({
          releasedBaseQuantity: fmtQ(
            asQ(lotAllocation.releasedBaseQuantity) + lotRemaining,
          ),
          status:
            asQ(lotAllocation.fulfilledBaseQuantity) > 0
              ? "fulfilled"
              : "released",
        })
  ...
  const [updated] = await tx
    ...
    .returning();
  await writeAudit(tx, actorId, organizationId, {
    action: `reservation.${terminalStatus}`,
    entityType: "inventory_reservation",
    entityId: reservationId,
    locationId: reservation.pantryLocationId,
    requestId,
    reason,
    previousValues: { status: reservation.status },
    newValues: { status: terminalStatus },
  });
  return updated;
}
...
export async function releaseReservation(
  ...
  return db.transaction(async (tx) => {
    const [reservation] = await tx
      .select({ pantryLocationId: inventoryReservations.pantryLocationId })
      .from(inventoryReservations)
      .where(
        and(
          eq(inventoryReservations.id, reservationId),
    ...
    await requireLocationPermission(
      tx,
      actorId,
      reservation.pantryLocationId,
      "reservation.release",
    );
    return releaseReservationInTx(
      tx,
      actorId,
      organizationId,
      reservationId,
    ...
    const due = await tx
      ...
      .where(
        and(
          ...
          sql`${inventoryReservations.expiresAt} is not null and ${inventoryReservations.expiresAt} <= now()`,
        ),
      );
...
export async function substituteReservationItem(
  ...
  return db.transaction(async (tx) => {
    const [line] = await tx
      ...
      .where(
        and(
          eq(inventoryReservationLines.id, reservationLineId),
          eq(inventoryReservationLines.organizationId, organizationId),
        ),
      )
      .for("update")
      .limit(1);
    ...
    await requireLocationPermission(
      tx,
      actorId,
      line.pantryLocationId,
      "pickup.substitute",
    );
    if (!line.isRequired && !line.appointmentAllocationLineId)
      throw new DomainError("SUBSTITUTION_NOT_ALLOWED");

    const [reservation] = await tx
      .select()
      .from(inventoryReservations)
      .where(eq(inventoryReservations.id, line.reservationId))
      .for("update")
      .limit(1);
    if (
      !reservation ||
      !["active", "partially_fulfilled"].includes(reservation.status)
    )
      throw new DomainError("RESERVATION_INVALID_STATE");
    if (asQ(line.fulfilledBaseQuantity) > 0)
      throw new DomainError("RESERVATION_INVALID_STATE");
    ...
    const [substitute] = await tx
      .select({
        ...
        categoryName: sql<
          string | null
        >`(select name from inventory_categories c where c.id = ${inventoryItems.categoryId})`,
      })
      .from(inventoryItems)
      .where(
        and(
          eq(inventoryItems.id, substituteItemId),
          eq(inventoryItems.organizationId, organizationId),
        ),
      )
      .limit(1);
    ...
    const restrictions = await tx
      .select({
        valueCode: householdPreferences.valueCode,
        severity: householdPreferences.severity,
      })
      .from(householdPreferences)
      .where(
        and(
          eq(householdPreferences.householdId, reservation.householdId),
          eq(householdPreferences.isActive, true),
          inArray(householdPreferences.preferenceType, ["dietary", "allergen"]),
        ),
      );
    const conflict = substitutionConflicts(
      restrictions,
      `${substitute.name} ${substitute.categoryName ?? ""}`,
    );
    if (conflict.blocked)
      throw new DomainError("SUBSTITUTION_DIETARY_CONFLICT");

    // Release the original line's remaining reservation.
    const remaining = Math.max(
      asQ(line.reservedBaseQuantity) - asQ(line.releasedBaseQuantity),
      0,
    );
    if (remaining <= 0) throw new DomainError("RESERVATION_INVALID_STATE");
    await tx
      .update(inventoryReservationLines)
      .set({ releasedBaseQuantity: line.reservedBaseQuantity })
      .where(eq(inventoryReservationLines.id, line.id));
    const lotAllocations = await tx
      .select()
      .from(inventoryReservationLotAllocations)
    ...
    for (const lotAllocation of lotAllocations) {
      await tx
        .update(inventoryReservationLotAllocations)
        .set({
          releasedBaseQuantity: lotAllocation.reservedBaseQuantity,
          status: "released",
        })
        .where(eq(inventoryReservationLotAllocations.id, lotAllocation.id));
    }
    ...
    if (plan.shortage > 0)
      throw new DomainError("RESERVATION_INSUFFICIENT_STOCK");
    ...
    const [substitution] = await tx
      ...
      .returning();
    await writeAudit(tx, actorId, organizationId, {
      action: "pickup.substitution_recorded",
      entityType: "pickup_substitution",
      entityId: substitution.id,
      locationId: line.pantryLocationId,
      requestId,
      reason,
      previousValues: { itemId: line.inventoryItemId },
      newValues: { itemId: substituteItemId, quantity: fmtQ(remaining) },
    });
...
async function checkInInTx(
  ...
  if (!checkInEligible(appointment.status))
    throw new DomainError("APPOINTMENT_INVALID_STATE");
  const [updated] = await tx
    .update(appointments)
...
export async function checkInAppointment(
  ...
  return db.transaction(async (tx) =>
    checkInInTx(tx, actorId, organizationId, appointmentId, requestId),
  );
}
...
export async function completePickup(
  ...
  return db.transaction(async (tx) => {
    ...
    const [existing] = await tx
      ...
      .where(
        and(
          eq(pickupFulfillments.organizationId, organizationId),
          eq(pickupFulfillments.idempotencyKey, values.idempotencyKey),
        ),
      )
      .limit(1);
    ...
    if (appointment.status === "completed")
      throw new DomainError("FULFILLMENT_ALREADY_COMPLETED");
    if (
    ...
    for (const input of values.lines) {
      ...
      const [line] = await tx
        ...
        .where(
          and(
            eq(inventoryReservationLines.id, input.reservationLineId),
            eq(inventoryReservationLines.reservationId, reservation.id),
          ),
        )
        .for("update")
        .limit(1);
      ...
      const [lotAllocation] = await tx
        ...
        .where(
          and(
            eq(inventoryReservationLotAllocations.reservationLineId, line.id),
            eq(
              inventoryReservationLotAllocations.inventoryLotId,
      ...
      const lotRemaining =
        ...
        asQ(lotAllocation.fulfilledBaseQuantity) -
        asQ(lotAllocation.releasedBaseQuantity);
      if (input.quantity > lotRemaining + 1e-9)
        throw new DomainError("FULFILLMENT_EXCEEDS_RESERVATION");
      ...
      const newLotFulfilled =
        asQ(lotAllocation.fulfilledBaseQuantity) + input.quantity;
      const exhausted =
        newLotFulfilled + asQ(lotAllocation.releasedBaseQuantity) >=
        asQ(lotAllocation.reservedBaseQuantity) - 1e-9;
      await tx
        .update(inventoryReservationLotAllocations)
        .set({
          fulfilledBaseQuantity: fmtQ(newLotFulfilled),
          status: exhausted ? "fulfilled" : "active",
        })
        .where(eq(inventoryReservationLotAllocations.id, lotAllocation.id));
      await tx
        .update(inventoryReservationLines)
        .set({
          fulfilledBaseQuantity: fmtQ(
            asQ(line.fulfilledBaseQuantity) + input.quantity,
          ),
        })
        .where(eq(inventoryReservationLines.id, line.id));
      anyFulfilled = true;
    ...
    for (const line of lines) {
      ...
      if (
        line.isRequired &&
        asQ(line.fulfilledBaseQuantity) + 1e-9 < asQ(line.reservedBaseQuantity)
      )
        allRequiredFullyFulfilled = false;
      const lotAllocations = await tx
        .select()
        .from(inventoryReservationLotAllocations)
        .where(
    ...
    const fulfillmentStatusValue =
      allRequiredFullyFulfilled && anyFulfilled
        ? "completed"
        : anyFulfilled
          ? "partially_completed"
          : "completed";
    const appointmentStatusValue: AppointmentStatus =
      allRequiredFullyFulfilled && anyFulfilled
        ? "completed"
        : anyFulfilled
          ? "partially_completed"
    ...
    await tx
      .update(inventoryReservations)
      .set({
        status: anyFulfilled ? "fulfilled" : "released",
        fulfilledAt: anyFulfilled ? new Date() : null,
        releasedAt: new Date(),
        releasedBy: actorId,
        releaseReason: anyFulfilled
          ? "Completed pickup"
          : "Completed with nothing fulfilled",
      })
...
export async function cancelAppointment(
  ...
  return db.transaction(async (tx) => {
    ...
    if (!cancellationEligible(appointment.status))
      throw new DomainError("APPOINTMENT_INVALID_STATE");
...
export async function rescheduleAppointment(
  ...
  return db.transaction(async (tx) => {
    ...
    if (!rescheduleEligible(appointment.status as AppointmentStatus))
      throw new DomainError("APPOINTMENT_INVALID_STATE");
    if (values.scheduledStartAt >= values.scheduledEndAt)
      throw new DomainError("APPOINTMENT_TIME_INVALID");
    ...
    await appendStatusHistory(
      ...
      "scheduled",
      actorId,
      `Rescheduled from ${appointment.appointmentNumber}`,
    );
    const [original] = await tx
      .update(appointments)
      .set({
        status: "rescheduled",
        rescheduledToAppointmentId: replacement.id,
      })
      .where(eq(appointments.id, appointmentId))
      .returning();
    await appendStatusHistory(
      ...
      "rescheduled",
      actorId,
      `Rescheduled to ${replacement.appointmentNumber}`,
    );
    await writeAudit(tx, actorId, organizationId, {
      ...
      locationId: appointment.pantryLocationId,
      requestId,
      previousValues: { startAt: appointment.scheduledStartAt.toISOString() },
      newValues: {
        replacementAppointmentId: replacement.id,
        startAt: values.scheduledStartAt.toISOString(),
      },
    });
 ...
 */
export async function correctPickupFulfillment(
  actorId: string,
  organizationId: string,
  ...
  return db.transaction(async (tx) => {
    const [fulfillment] = await tx
      ...
      .where(
        and(
          eq(pickupFulfillments.id, fulfillmentId),
          eq(pickupFulfillments.organizationId, organizationId),
        ),
      )
      .for("update")
    ...
    await requireLocationPermission(
      tx,
      actorId,
      fulfillment.pantryLocationId,
      "pickup.correct",
    );
    if (fulfillment.status === "corrected") return fulfillment; // idempotent
    if (
      fulfillment.status !== "completed" &&
      fulfillment.status !== "partially_completed"
    )
      throw new DomainError("FULFILLMENT_IMMUTABLE");
    const membershipId = await actorMembership(tx, actorId, organizationId);

    const lines = await tx
      .select()
      .from(pickupFulfillmentLines)
      .where(eq(pickupFulfillmentLines.pickupFulfillmentId, fulfillmentId));
    for (const line of lines) {
    ...
    const [corrected] = await tx
      ...
      .returning();
    await writeAudit(tx, actorId, organizationId, {
      action: "pickup.corrected",
      entityType: "pickup_fulfillment",
      entityId: fulfillmentId,
      locationId: fulfillment.pantryLocationId,
      requestId,
      reason,
      previousValues: { status: fulfillment.status },
      newValues: { status: "corrected", reversedLines: lines.length },
    });
    return corrected;
  });
}
