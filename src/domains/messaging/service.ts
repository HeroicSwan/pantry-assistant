...
async function requireLocationScope(
  executor: Executor,
  actorId: string,
  ...
  const [allowed, scope] = await Promise.all([
    ...
    executor.execute<{ valid: boolean }>(
      sql`select exists(select 1 from pantry_locations where id=${locationId} and organization_id=${organizationId} and status<>'archived') valid`,
    ),
  ]);
  if (!allowed || !scope.rows[0]?.valid) throw new DomainError("FORBIDDEN");
}

async function requireAnyLocationPermission(
  executor: Executor,
  actorId: string,
  organizationId: string,
  locationId: string,
  ...
  const scope = await executor.execute<{ valid: boolean }>(
    sql`select exists(select 1 from pantry_locations where id=${locationId} and organization_id=${organizationId} and status<>'archived') valid`,
  );
  if (!scope.rows[0]?.valid) throw new DomainError("FORBIDDEN");
  for (const permission of permissions)
    if (await hasLocationPermission(executor, actorId, locationId, permission))
      return;
...
async function writeAudit(
  ...
  const membership = await tx.execute<{ id: string }>(
    sql`select id from organization_memberships where user_id=${actorId} and organization_id=${organizationId} and status='active' and archived_at is null limit 1`,
  );
  await tx.execute(
    sql`insert into audit_logs(organization_id,location_id,actor_user_id,actor_membership_id,action,entity_type,entity_id,request_id,new_values) values(${organizationId},${locationId},${actorId},${membership.rows[0]?.id ?? null},${action},${entityType},${entityId},${requestId},${JSON.stringify(values)}::jsonb)`,
  );
}
...
export async function saveMessagingSettings(
  ...
  return db.transaction(async (tx) => {
    ...
    await requireLocationScope(
      tx,
      actorId,
      organizationId,
      locationId,
      "message.settings.manage",
    );
    if (
      values.sendingMode === "live" &&
      (!values.confirmLive ||
        !providerHasCredentials(provider) ||
        (!values.defaultFromNumber && provider !== "aws_sns"))
    )
      throw new DomainError("SMS_LIVE_CONFIGURATION_REQUIRED");
    if (
      values.reminderHoursBefore < 1 ||
      values.reminderHoursBefore > 168 ||
      values.retryLimit < 0 ||
      values.retryLimit > 10
    )
      throw new DomainError("VALIDATION_ERROR");
    const from = values.defaultFromNumber
      ? normalizePhoneNumber(values.defaultFromNumber)
      : null;
    if (values.defaultFromNumber && !from)
      throw new DomainError("CONSENT_INVALID");
    ...
    await writeAudit(
      ...
      result.rows[0].id,
      requestId,
      { provider, sendingMode: values.sendingMode, enabled: values.isEnabled },
    );
...
export async function createMessageTemplate(
  ...
  return db.transaction(async (tx) => {
    ...
    if (!values.name.trim() || !values.body.trim() || rendered.segments > 10)
      throw new DomainError("VALIDATION_ERROR");
    const result = await tx.execute<{ id: string }>(
      sql`insert into message_templates(organization_id,pantry_location_id,name,template_type,language,body,variables,created_by) values(${organizationId},${locationId},${values.name.trim()},${values.templateType},${values.language},${values.body},${JSON.stringify(rendered.usedVariables)}::jsonb,${actorId}) returning id`,
    );
    await writeAudit(
      tx,
      actorId,
      ...
      result.rows[0].id,
      requestId,
      { name: values.name, templateType: values.templateType },
    );
    return result.rows[0];
  });
}
...
export async function archiveMessageTemplate(
  ...
  return db.transaction(async (tx) => {
    await requireLocationScope(
      ...
      organizationId,
      locationId,
      "message.template.archive",
    );
    const result = await tx.execute<{ id: string }>(
      sql`update message_templates set status='archived',archived_at=now(),updated_at=now() where id=${templateId} and organization_id=${organizationId} and (pantry_location_id=${locationId} or pantry_location_id is null) and archived_at is null returning id`,
    );
...
export async function updateMessageTemplate(
  ...
  return db.transaction(async (tx) => {
    ...
    if (!values.name.trim() || !values.body.trim() || rendered.segments > 10)
      throw new DomainError("VALIDATION_ERROR");
    const result = await tx.execute<{ id: string }>(
      sql`update message_templates set name=${values.name.trim()},template_type=${values.templateType},language=${values.language},body=${values.body},variables=${JSON.stringify(rendered.usedVariables)}::jsonb,updated_at=now() where id=${templateId} and organization_id=${organizationId} and pantry_location_id=${locationId} and is_system_template=false and archived_at is null returning id`,
    );
    if (!result.rows[0]) throw new DomainError("NOT_FOUND");
    await writeAudit(
      tx,
...
export async function previewIndividualMessage(
  actorId: string,
  organizationId: string,
  locationId: string,
  input: IndividualMessageInput,
) {
  await requireAnyLocationPermission(db, actorId, organizationId, locationId, [
    "message.draft",
    "message.send_individual",
  ]);
  ...
  if (input.templateId) {
    const template = await db.execute<{ body: string; language: string }>(
      sql`select body,language from message_templates where id=${input.templateId} and organization_id=${organizationId} and (pantry_location_id=${locationId} or pantry_location_id is null) and status='active' and archived_at is null`,
    );
  ...
  return {
    ...
    eligibility,
    body: rendered.body,
    language: input.language ?? eligibility.preferredLanguage,
...
export async function sendIndividualMessage(
  ...
  organizationId: string,
  locationId: string,
  input: IndividualMessageInput & { confirmed: boolean },
  requestId: string,
) {
  ...
  const preview = await previewIndividualMessage(
    actorId,
    organizationId,
    locationId,
    input,
  ...
  if (
    ...
    !preview.eligibility.normalizedPhoneNumber
  )
    throw new DomainError("MESSAGE_RECIPIENT_INELIGIBLE");
  const settings = await settingsFor(db, organizationId, locationId);
  if (!settings || !settings.is_enabled || settings.sending_mode === "disabled")
    throw new DomainError("SMS_DISABLED");
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const id =
      ...
      (
        await tx.execute<{ id: string }>(
          sql`select id from sms_messages where organization_id=${organizationId} and idempotency_key=${input.idempotencyKey}`,
        )
      ).rows[0]?.id;
    ...
    await writeAudit(
      ...
      organizationId,
      locationId,
      scheduled ? "message.scheduled" : "message.queued",
      "sms_message",
      id,
      requestId,
      { segments: preview.segments, mode: settings.sending_mode },
    );
    return { id, scheduled };
  });
...
async function claimMessageForDispatch(messageId: string) {
  return db.transaction(async (tx) => {
    ...
    if (row.scheduled_for && new Date(String(row.scheduled_for)) > new Date())
      return null;
    if (row.campaign_status === "cancelled") {
      await tx.execute(
        sql`update sms_messages set status='cancelled',updated_at=now() where id=${messageId}`,
      );
      return null;
    }
    if (!row.is_enabled || row.sending_mode === "disabled") {
      await tx.execute(
        sql`update sms_messages set status='failed',failed_at=now(),provider_error_code='SMS_DISABLED',provider_error_message='Messaging is disabled.',updated_at=now() where id=${messageId}`,
      );
      return null;
    }
    const eligibility = evaluateSmsRecipientEligibility({
      phoneNumber: String(row.to_phone_number),
      consentId: row.current_consent_id ? String(row.current_consent_id) : null,
      consentStatus: row.current_consent_status
        ? String(row.current_consent_status)
        : null,
      householdStatus: row.household_status
        ? String(row.household_status)
        : null,
      contactActive: row.contact_active === true,
      contactArchived: Boolean(row.contact_archived_at),
      contactOrganizationId: String(row.organization_id),
      expectedOrganizationId: String(row.organization_id),
      contactLocationId: row.default_pantry_location_id
        ? String(row.default_pantry_location_id)
        : null,
      expectedLocationId: String(row.pantry_location_id),
    });
    if (!eligibility.eligible || !eligibility.normalizedPhoneNumber) {
      await tx.execute(
        sql`update sms_messages set status='excluded',provider_error_code=${eligibility.exclusionReason},provider_error_message='Recipient failed send-time eligibility.',updated_at=now() where id=${messageId}`,
      );
      return null;
    }
    if (
      isWithinQuietHours(
        new Date(),
        row.quiet_hours_start ? String(row.quiet_hours_start) : null,
        row.quiet_hours_end ? String(row.quiet_hours_end) : null,
        String(row.timezone),
      )
    ) {
      await tx.execute(
        sql`update sms_messages set status='scheduled',scheduled_for=now()+interval '30 minutes',updated_at=now() where id=${messageId}`,
      );
      return null;
    }
    await tx.execute(
      sql`update sms_messages set status='sending',queued_at=coalesce(queued_at,now()),attempt_count=attempt_count+1,updated_at=now() where id=${messageId}`,
    );
    return {
      ...
      default_from_number: row.default_from_number
        ? String(row.default_from_number)
        : null,
...
export async function dispatchMessage(messageId: string) {
  ...
  const result = await providerForMode(mode, String(row.provider)).sendMessage({
    to: String(row.to_phone_number),
    body: String(row.body_snapshot),
    from: row.default_from_number ? String(row.default_from_number) : null,
...
export async function retryFailedMessage(
  ...
  if (
    ...
    throw new DomainError("MESSAGE_RETRY_NOT_ALLOWED");
  await db.transaction(async (tx) => {
    await tx.execute(
      sql`update sms_messages set status='queued',scheduled_for=null,updated_at=now() where id=${messageId}`,
    );
    await writeAudit(
      tx,
      actorId,
...
export async function processStatusWebhook(
  ...
  const message = await db.execute<{
    ...
    sql`select id,organization_id,pantry_location_id,status from sms_messages where provider=${provider} and provider_message_id=${event.providerMessageId} limit 1`,
  );
  const row = message.rows[0];
  if (!row) return { found: false, updated: false };
  return db.transaction(async (tx) => {
    const inserted = await tx.execute<{ id: string }>(
      sql`insert into sms_events(organization_id,pantry_location_id,sms_message_id,provider_event_id,event_type,provider_status,payload_snapshot,received_at,processed_at) values(${row.organization_id},${row.pantry_location_id},${row.id},${event.providerEventId},'provider_status',${event.status},${JSON.stringify(event.payload)}::jsonb,now(),now()) on conflict(organization_id,provider_event_id) do nothing returning id`,
    );
    if (!inserted.rows[0]) return { found: true, updated: false };
    const locked = await tx.execute<{ status: string }>(
      sql`select status from sms_messages where id=${row.id} for update`,
    );
    const current = locked.rows[0]?.status ?? row.status;
    if (!canAdvanceProviderStatus(current, event.status))
...
type AudienceRecipient = ContactRecipient & {
...
export async function previewCampaignAudience(
  ...
  await requireAnyLocationPermission(db, actorId, organizationId, locationId, [
    "message.draft",
    "message.send_bulk",
    "message.approve_bulk",
  ]);
  const rows = deduplicateRecipients(
    (await audienceRows(organizationId, locationId, audience)).map((row) => ({
      ...row,
      phoneNumber: row.phone_normalized ?? row.phone_number,
    })),
  );
  const recipients = rows.map((row) => ({
    ...
    eligibility: evaluateSmsRecipientEligibility({
      ...
      appointmentStatus: audience.appointmentDate
  ...
  const byReason = Object.fromEntries(
    [
      ...new Set(
        exclusions.map((row) => row.eligibility.exclusionReason ?? "other"),
      ),
    ].map((reason) => [
      reason,
      exclusions.filter((row) => row.eligibility.exclusionReason === reason)
        .length,
    ]),
  );
  return {
    matched: recipients.length,
    eligible: recipients.filter((row) => row.eligibility.eligible),
    exclusions,
    byReason,
  };
}
...
export async function createMessageCampaign(
  ...
  return db.transaction(async (tx) => {
    ...
    await writeAudit(
      ...
      id,
      requestId,
      { name: values.name, segments: segments.segments },
    );
    return { id };
  });
}

export async function transitionMessageCampaign(
  actorId: string,
  organizationId: string,
  locationId: string,
  campaignId: string,
  target: "awaiting_approval" | "approved" | "cancelled",
  reason: string | null,
  requestId: string,
) {
  const permission =
  ...
  return db.transaction(async (tx) => {
    ...
    const result = await tx.execute<{ status: string; created_by: string }>(
      sql`select status,created_by from message_campaigns where id=${campaignId} and organization_id=${organizationId} and pantry_location_id=${locationId} for update`,
    );
    const campaign = result.rows[0];
    if (!campaign) throw new DomainError("NOT_FOUND");
    if (!canTransitionCampaign(campaign.status, target))
      throw new DomainError("MESSAGE_CAMPAIGN_INVALID_STATE");
    if (target === "cancelled" && (!reason || reason.trim().length < 3))
      throw new DomainError("REASON_REQUIRED");
    await tx.execute(
      sql`update message_campaigns set status=${target},approved_by=case when ${target}='approved' then ${actorId} else approved_by end,approved_at=case when ${target}='approved' then now() else approved_at end,cancelled_by=case when ${target}='cancelled' then ${actorId} else cancelled_by end,cancelled_at=case when ${target}='cancelled' then now() else cancelled_at end,cancellation_reason=case when ${target}='cancelled' then ${reason} else cancellation_reason end,updated_at=now() where id=${campaignId}`,
    );
    if (target === "cancelled")
      await tx.execute(
        sql`update sms_messages set status='cancelled',updated_at=now() where campaign_id=${campaignId} and status in('draft','scheduled','queued','accepted')`,
      );
    await writeAudit(
      tx,
      actorId,
...
export async function sendMessageCampaign(
  ...
  const campaignResult = await db.execute<{
    id: string;
    status: string;
    audience_definition: CampaignAudience;
    message_body_snapshot: string;
    scheduled_for: string | null;
  }>(
    sql`select id,status,audience_definition,message_body_snapshot,scheduled_for::text from message_campaigns where id=${campaignId} and organization_id=${organizationId} and pantry_location_id=${locationId}`,
  );
  const campaign = campaignResult.rows[0];
  if (
    !campaign ||
    !["approved", "scheduled", "partially_sent", "failed"].includes(
      campaign.status,
    )
  )
    throw new DomainError(
      campaign ? "MESSAGE_CAMPAIGN_INVALID_STATE" : "NOT_FOUND",
    );
  ...
  const beganSending = await db.transaction(async (tx) => {
    const locked = await tx.execute<{ status: string }>(
      sql`select status from message_campaigns where id=${campaignId} for update`,
    );
    if (
      locked.rows[0]?.status === "cancelled" ||
      locked.rows[0]?.status === "sent"
    )
      return false;
    await tx.execute(
      sql`update message_campaigns set status='sending',updated_at=now() where id=${campaignId}`,
    );
    await tx.execute(
      sql`delete from message_recipient_exclusions where campaign_id=${campaignId}`,
    );
    for (const row of preview.exclusions)
      await tx.execute(
        sql`insert into message_recipient_exclusions(campaign_id,household_id,contact_id,phone_number,exclusion_reason) values(${campaignId},${row.household_id},${row.contact_id},${row.normalizedPhoneNumber},${row.eligibility.exclusionReason ?? "other"})`,
      );
    for (const row of preview.eligible) {
      if (!row.eligibility.normalizedPhoneNumber) continue;
      const idempotency = deterministicUuid(
        `campaign:${campaignId}:${row.eligibility.normalizedPhoneNumber}`,
      );
      await tx.execute(
        sql`insert into sms_messages(organization_id,pantry_location_id,campaign_id,household_id,household_contact_id,consent_id,direction,message_type,status,to_phone_number,from_phone_number,body_snapshot,language,scheduled_for,queued_at,provider,idempotency_key,created_by) values(${organizationId},${locationId},${campaignId},${row.household_id},${row.contact_id},${row.eligibility.consentId},'outbound','bulk_announcement',${campaign.scheduled_for && new Date(campaign.scheduled_for) > new Date() ? "scheduled" : "queued"},${row.eligibility.normalizedPhoneNumber},${settings.default_from_number},${campaign.message_body_snapshot},${row.eligibility.preferredLanguage},${campaign.scheduled_for},${campaign.scheduled_for ? null : new Date()},${settings.sending_mode === "simulation" ? "simulation" : settings.provider},${idempotency},${actorId}) on conflict(organization_id,idempotency_key) do nothing`,
      );
    }
    await writeAudit(
      tx,
  ...
  const futureSchedule = Boolean(
    campaign.scheduled_for && new Date(campaign.scheduled_for) > new Date(),
  );
  if (futureSchedule) {
    await db.execute(
      sql`update message_campaigns set status='scheduled',updated_at=now() where id=${campaignId} and status='sending'`,
    );
    return {
  ...
  const dispatchable = await db.execute<{ id: string }>(
    sql`select id from sms_messages where campaign_id=${campaignId} and status='queued' order by created_at limit 100`,
  );
  ...
  const metrics = await db.execute<{
    ...
    sql`select count(*)::int total,count(*) filter(where status in('sent','delivered'))::int completed,count(*) filter(where status in('failed','undelivered','excluded'))::int failed from sms_messages where campaign_id=${campaignId}`,
  );
    ...
    row.total === 0
      ...
      : row.completed + row.failed < row.total
        ...
        : row.failed > 0
          ...
          : "sent";
  await db.execute(
    sql`update message_campaigns set status=${nextStatus},updated_at=now() where id=${campaignId} and status='sending'`,
  );
  return { ...row, status: nextStatus, excluded: preview.exclusions.length };
}

function inboundResponse(
  intent: ReturnType<typeof parseInboundIntent>,
  helpResponse: string,
  recognizedContact: boolean,
  activeConsent: boolean,
) {
  if (intent === "stop")
    return "You have been opted out. Reply START to request messages again.";
  if (intent === "start")
    return recognizedContact
      ? "Your request to receive pantry messages was recorded. Reply STOP to opt out."
      : "We could not match this number. Please contact the pantry for help.";
  if (intent === "help") return helpResponse;
  if (intent === "confirm" && activeConsent)
    return "Your pantry appointment is confirmed.";
  if (intent === "cancellation_intent" && activeConsent)
    return "Your cancellation request was received for staff review.";
  return null;
}
...
export async function processInboundWebhook(event: InboundSmsEvent) {
  ...
  if (!from || !to || !event.providerMessageId || event.body.length > 1600)
    throw new DomainError("VALIDATION_ERROR");
  const settingResult = await db.execute<MessagingSettings>(
    sql`select s.*,coalesce(l.timezone,o.timezone,'UTC') timezone from sms_settings s join pantry_locations l on l.id=s.pantry_location_id join organizations o on o.id=s.organization_id where s.is_enabled and regexp_replace(coalesce(s.default_from_number,''),'[^0-9]','','g')=${to.replace(/\D/g, "")} limit 2`,
  );
  if (settingResult.rows.length !== 1)
    return { accepted: true, response: null, matched: false };
  ...
  const transactionResult = await db.transaction(async (tx) => {
    ...
    if (intent === "stop" && contacts.rows.length) {
      for (const row of contacts.rows)
        if (row.consent_id && row.consent_status !== "opted_out")
          await tx.execute(
            sql`insert into sms_consents(organization_id,household_id,household_contact_id,phone_normalized,status,consent_source,recorded_by,notes) values(${settings.organization_id},${row.household_id},${row.contact_id},${from},'opted_out','administrative_correction',${settings.created_by},'Inbound STOP command')`,
          );
    } else if (
      intent === "start" &&
      contact?.consent_id &&
      ...
      await tx.execute(
        sql`insert into sms_consents(organization_id,household_id,household_contact_id,phone_normalized,status,consent_source,recorded_by,notes) values(${settings.organization_id},${contact.household_id},${contact.contact_id},${from},'consented','inbound_start',${settings.created_by},'Inbound START command')`,
      );
    } else if (
      (intent === "confirm" || intent === "cancellation_intent") &&
      contact
    ) {
      const appointment = await tx.execute<{ id: string; status: string }>(
        sql`select id,status::text from appointments where organization_id=${settings.organization_id} and pantry_location_id=${settings.pantry_location_id} and household_id=${contact.household_id} and status in('scheduled','confirmed') and scheduled_end_at>now() order by scheduled_start_at limit 1 for update`,
      );
      appointmentId = appointment.rows[0]?.id ?? null;
      if (
        appointmentId &&
        intent === "confirm" &&
        appointment.rows[0].status === "scheduled"
      ) {
        await tx.execute(
          sql`update appointments set status='confirmed',updated_at=now() where id=${appointmentId}`,
        );
        await tx.execute(
          sql`insert into appointment_status_history(organization_id,pantry_location_id,appointment_id,from_status,to_status,reason,changed_by) values(${settings.organization_id},${settings.pantry_location_id},${appointmentId},'scheduled','confirmed','Confirmed by inbound SMS',${settings.created_by})`,
        );
      }
    }
    const processing =
      intent === "unknown" || intent === "cancellation_intent" || !contact
        ? "review_required"
        : "processed";
    await tx.execute(
      sql`update inbound_messages set processing_status=${processing},linked_appointment_id=${appointmentId} where id=${inboundId}`,
    );
    if (response) {
      await tx.execute(
        sql`insert into sms_compliance_messages(organization_id,pantry_location_id,inbound_message_id,to_phone_number,from_phone_number,body_snapshot,command) values(${settings.organization_id},${settings.pantry_location_id},${inboundId},${from},${to},${response},${intent}) on conflict(inbound_message_id) do nothing`,
      );
    }
    await writeAudit(
      tx,
      ...
      settings.pantry_location_id,
      `message.inbound.${intent}`,
      "inbound_message",
      inboundId,
      deterministicUuid(`inbound-audit:${event.providerMessageId}`),
      { matched: Boolean(contact), appointmentId },
    );
...
export async function scheduleAppointmentReminders(
  ...
  for (const row of candidates.rows) {
    const eligibility = evaluateSmsRecipientEligibility({
      phoneNumber: String(row.phone_normalized),
      consentId: String(row.consent_id ?? "") || null,
      consentStatus: String(row.consent_status ?? "") || null,
...
export async function dispatchDueMessages(limit = 100) {
  const due = await db.execute<{ id: string; campaign_id: string | null }>(
    sql`select id,campaign_id from sms_messages where status in('queued','scheduled') and (scheduled_for is null or scheduled_for<=now()) order by coalesce(scheduled_for,queued_at,created_at) limit ${Math.max(1, Math.min(limit, 500))} for update skip locked`,
  );
  const results = await Promise.all(
    due.rows.map((row) => dispatchMessage(row.id)),
  );
  const processed = results.filter(Boolean).length;
  for (const campaignId of new Set(
    due.rows.flatMap((row) => (row.campaign_id ? [row.campaign_id] : [])),
  )) {
    const metrics = await db.execute<{
      total: number;
      completed: number;
    ...
    const status =
      row.completed + row.failed < row.total
        ...
        : row.failed > 0
          ...
          : "sent";
    await db.execute(
      sql`update message_campaigns set status=${status},updated_at=now() where id=${campaignId} and status in('scheduled','sending','partially_sent')`,
    );
  }
...
export async function retryDueMessages(limit = 100) {
  ...
  for (const row of failed.rows) {
    ...
    if (
      ...
      retryDelaySeconds(row.attempt_count) * 1000
    )
      continue;
    await db.execute(
      sql`update sms_messages set status='queued',updated_at=now() where id=${row.id}`,
    );
...
export async function markInboundHandled(
  ...
  return db.transaction(async (tx) => {
    await requireLocationScope(
      ...
      organizationId,
      locationId,
      "message.manage_inbound",
    );
    const result = await tx.execute<{ id: string }>(
      sql`update inbound_messages set processing_status='handled',handled_by=${actorId},handled_at=now() where id=${inboundId} and organization_id=${organizationId} and pantry_location_id=${locationId} and handled_at is null returning id`,
    );
    if (!result.rows[0]) throw new DomainError("NOT_FOUND");
    await writeAudit(
      tx,
