...
export const GENERIC_WEBHOOK_PROVIDER_IDS = SMS_PROVIDER_IDS.filter(
  (provider) => provider !== "twilio",
) as Exclude<SmsProviderId, "twilio">[];
...
async function requestParameters(request: Request) {
  ...
  const entries = [...form.entries()].map(
...
export async function validateTwilioWebhookRequest(
  ...
  const expected = createTwilioSignature(
    externallyVisibleWebhookUrl(request),
    entries,
    authToken,
  );
...
export function validateGenericWebhookRequest(
  request: Request,
  secret?: string,
) {
  const provided =
    request.headers.get("x-pantry-webhook-secret") ??
...
export function genericWebhookSecretFor(
  provider: Exclude<SmsProviderId, "twilio">,
) {
  const key = `SMS_WEBHOOK_SECRET_${provider.toUpperCase().replaceAll(/[^A-Z0-9]/g, "_")}`;
...
export class SimulationSmsProvider implements SmsProvider {
  async sendMessage(input: SendSmsInput): Promise<SendSmsResult> {
    const marker = input.body
      .match(/\[simulate:(queued|accepted|sent|delivered|failed)]/i)?.[1]
      ?.toLowerCase();
    ...
    return {
      ...
      status: failed
        ? "failed"
        : ((marker as SendSmsResult["status"] | undefined) ?? "delivered"),
  ...
  async parseStatusWebhook(request: Request): Promise<SmsStatusEvent> {
    ...
    return {
      providerEventId: deterministicUuid(
        `simulation-status:${providerMessageId}:${status}:${values.ErrorCode ?? ""}`,
      ),
      providerMessageId,
      status,
  ...
  async parseInboundWebhook(request: Request): Promise<InboundSmsEvent> {
    const { values } = await requestParameters(request);
    return {
      providerMessageId: values.MessageSid ?? values.SmsSid ?? "",
      from: values.From ?? "",
      to: values.To ?? "",
      body: values.Body ?? "",
      payload: safeTwilioPayload(values),
  ...
  async sendMessage(input: SendSmsInput): Promise<SendSmsResult> {
    ...
    if (!to)
      return {
        ...
        errorMessage: "The recipient number is invalid.",
        simulated: false,
      };
    const form = new URLSearchParams({ To: to, Body: input.body });
    const serviceSid =
      input.messagingServiceSid ?? this.credentials.messagingServiceSid;
    ...
    if (serviceSid) form.set("MessagingServiceSid", serviceSid);
      ...
      return {
        ...
        providerMessageId: null,
        status: "failed",
        errorCode: "SENDER_MISSING",
        errorMessage: "A Twilio sender is not configured.",
        simulated: false,
      };
    try {
      ...
      const status = ["queued", "accepted", "sent", "delivered"].includes(
        payload.status ?? "",
      )
        ? (payload.status as SendSmsResult["status"])
        : "accepted";
      return {
        provider: "twilio",
        providerMessageId: payload.sid ?? null,
  ...
  async parseStatusWebhook(request: Request): Promise<SmsStatusEvent> {
    ...
    return {
      providerEventId:
        values.EventSid ??
        deterministicUuid(
          `twilio-status:${providerMessageId}:${status}:${values.ErrorCode ?? ""}`,
        ),
      providerMessageId,
      status,
      errorCode: values.ErrorCode ?? null,
...
function accepted(
  provider: SmsProviderId,
  providerMessageId: string | null,
  status: SendSmsResult["status"] = "accepted",
): SendSmsResult {
  return {
    provider,
    providerMessageId,
...
async function fetchProvider(
  ...
  url: string,
  init: RequestInit,
  messageField: (payload: Record<string, unknown>) => string | null,
  statusField?: (payload: Record<string, unknown>) => SendSmsResult["status"],
) {
  try {
    ...
    if (!response.ok)
      return failed(
        provider,
        String(payload.code ?? payload.errorCode ?? response.status),
        String(
          payload.message ?? payload.error ?? "Provider rejected the message.",
        ),
      );
    return accepted(
      provider,
      messageField(payload),
      statusField?.(payload) ?? "accepted",
    );
  } catch (error) {
    const timeout =
      error instanceof Error &&
...
function requireSender(input: SendSmsInput, provider: SmsProviderId) {
  ...
  return sender
    ...
    : failed(
        provider,
        "SENDER_MISSING",
        "A sender is required for this SMS provider.",
      );
}
...
class VonageSmsProvider implements SmsProvider {
  async sendMessage(input: SendSmsInput) {
    ...
    const sender = requireSender(input, "vonage");
    if (!apiKey || !apiSecret)
      return failed(
        "vonage",
        "CONFIGURATION_MISSING",
        "Vonage credentials are not configured.",
      );
    ...
    const form = new URLSearchParams({
      ...
      from: sender,
      to: normalizePhoneNumber(input.to) ?? input.to,
      text: input.body,
    });
  ...
  async parseStatusWebhook(request: Request) {
    return parseGenericStatus(request, "vonage");
  }
  async parseInboundWebhook(request: Request) {
    return parseGenericInbound(request);
  }
}
...
class PlivoSmsProvider implements SmsProvider {
  async sendMessage(input: SendSmsInput) {
    ...
    const sender = requireSender(input, "plivo");
    if (!authId || !authToken)
      return failed(
        "plivo",
        "CONFIGURATION_MISSING",
    ...
    return fetchProvider(
        ...
        "/Message/",
      {
        ...
        body: JSON.stringify({
          src: sender,
          dst: normalizePhoneNumber(input.to) ?? input.to,
          text: input.body,
        }),
      },
      (payload) =>
        String(
          (payload.message_uuid as string[] | undefined)?.[0] ??
  ...
  async parseStatusWebhook(request: Request) {
    return parseGenericStatus(request, "plivo");
  }
  async parseInboundWebhook(request: Request) {
    return parseGenericInbound(request);
  }
...
class TelnyxSmsProvider implements SmsProvider {
  async sendMessage(input: SendSmsInput) {
    ...
    const sender = requireSender(input, "telnyx");
    if (!apiKey || !profile)
      return failed(
        "telnyx",
        "CONFIGURATION_MISSING",
        "Telnyx credentials are not configured.",
      );
    if (typeof sender !== "string") return sender;
    return fetchProvider(
      "telnyx",
      ...
      (payload) =>
        String(
          (payload.data as Record<string, unknown> | undefined)?.id ?? null,
        ),
    );
  }
  validateWebhook() {
  ...
  async parseStatusWebhook(request: Request) {
    return parseGenericStatus(request, "telnyx");
  }
  async parseInboundWebhook(request: Request) {
    return parseGenericInbound(request);
  }
...
class SinchSmsProvider implements SmsProvider {
  async sendMessage(input: SendSmsInput) {
    ...
    const sender = requireSender(input, "sinch");
    if (!plan || !token)
      return failed(
        "sinch",
        "CONFIGURATION_MISSING",
        "Sinch credentials are not configured.",
      );
    if (typeof sender !== "string") return sender;
    return fetchProvider(
      "sinch",
  ...
  async parseStatusWebhook(request: Request) {
    return parseGenericStatus(request, "sinch");
  }
  async parseInboundWebhook(request: Request) {
    return parseGenericInbound(request);
  }
...
class InfobipSmsProvider implements SmsProvider {
  async sendMessage(input: SendSmsInput) {
    ...
    const sender = requireSender(input, "infobip");
    if (!base || !apiKey)
      return failed(
        "infobip",
        "CONFIGURATION_MISSING",
    ...
    return fetchProvider(
      "infobip",
      base.replace(/\/$/, "") + "/sms/2/text/advanced",
      {
        method: "POST",
        headers: {
      ...
      (payload) =>
        String(
          ((payload.messages as Array<Record<string, unknown>> | undefined)?.[0]
            ?.messageId as string | undefined) ?? null,
        ),
    );
  }
  validateWebhook() {
  ...
  async parseStatusWebhook(request: Request) {
    return parseGenericStatus(request, "infobip");
  }
  async parseInboundWebhook(request: Request) {
    return parseGenericInbound(request);
  }
...
class BandwidthSmsProvider implements SmsProvider {
  async sendMessage(input: SendSmsInput) {
    const token = process.env.BANDWIDTH_API_TOKEN;
    const secret = process.env.BANDWIDTH_API_SECRET;
    const applicationId = process.env.BANDWIDTH_APPLICATION_ID;
    const sender = requireSender(input, "bandwidth");
    if (!token || !secret || !applicationId)
      return failed(
        "bandwidth",
        "CONFIGURATION_MISSING",
        "Bandwidth credentials are not configured.",
      );
    if (typeof sender !== "string") return sender;
    return fetchProvider(
      "bandwidth",
      "https://messaging.bandwidth.com/api/v2/messages",
      {
        method: "POST",
        headers: {
          authorization:
  ...
  async parseStatusWebhook(request: Request) {
    return parseGenericStatus(request, "bandwidth");
  }
  async parseInboundWebhook(request: Request) {
    return parseGenericInbound(request);
  }
...
class BirdSmsProvider implements SmsProvider {
  async sendMessage(input: SendSmsInput) {
    ...
    if (!accessKey || !workspaceId || !channelId)
      return failed(
        "bird",
        "CONFIGURATION_MISSING",
        "Bird credentials are not configured.",
      );
    return fetchProvider(
      "bird",
      "https://api.bird.com/workspaces/" +
        ...
        "/messages",
      {
        ...
        body: JSON.stringify({
          receiver: {
            contacts: [
              { identifier: normalizePhoneNumber(input.to) ?? input.to },
            ],
          },
          body: { type: "text", text: { text: input.body } },
        }),
      },
      (payload) => String(payload.id ?? null),
    );
  ...
  async parseStatusWebhook(request: Request) {
    return parseGenericStatus(request, "bird");
  }
  async parseInboundWebhook(request: Request) {
    return parseGenericInbound(request);
  }
...
class AwsSnsSmsProvider implements SmsProvider {
  async sendMessage(input: SendSmsInput) {
    ...
    if (!region || !accessKeyId || !secretAccessKey)
      return failed(
        "aws_sns",
        "CONFIGURATION_MISSING",
        "AWS SNS credentials are not configured.",
      );
    try {
      ...
      const result = await client.send(
        new PublishCommand({
          PhoneNumber: normalizePhoneNumber(input.to) ?? input.to,
          Message: input.body,
          MessageAttributes: process.env.AWS_SNS_SENDER_ID
            ? {
  ...
  async parseStatusWebhook(request: Request) {
    return parseGenericStatus(request, "aws_sns");
  }
  async parseInboundWebhook(request: Request) {
    return parseGenericInbound(request);
  }
...
class AzureCommunicationSmsProvider implements SmsProvider {
  async sendMessage(input: SendSmsInput) {
    ...
    const sender = requireSender(input, "azure_communication_services");
    if (!connection)
      return failed(
        "azure_communication_services",
        "CONFIGURATION_MISSING",
    ...
    if (typeof sender !== "string") return sender;
    const endpoint = connection.match(/endpoint=([^;]+)/i)?.[1];
    const accessKey = connection.match(/accesskey=([^;]+)/i)?.[1];
    if (!endpoint || !accessKey)
      return failed(
        "azure_communication_services",
        "CONFIGURATION_INVALID",
        "Azure Communication Services connection string is invalid.",
      );
    const url = new URL(
      endpoint.replace(/\/$/, "") + "/sms?api-version=2026-01-23",
    );
    const body = JSON.stringify({
      from: sender,
      smsRecipients: [{ to: normalizePhoneNumber(input.to) ?? input.to }],
      message: input.body,
    });
  ...
  async parseStatusWebhook(request: Request) {
    return parseGenericStatus(request, "azure_communication_services");
  }
  async parseInboundWebhook(request: Request) {
    return parseGenericInbound(request);
  }
...
async function parseGenericStatus(
  request: Request,
  provider: SmsProviderId,
): Promise<SmsStatusEvent> {
  const values = await parseGenericJson(request);
  ...
  const status = String(
    values.status ?? values.messageStatus ?? values.MessageStatus ?? "",
  ).toLowerCase();
  return {
    providerEventId: String(
      values.eventId ??
        values.event_id ??
        deterministicUuid(provider + ":" + providerMessageId + ":" + status),
    ),
    providerMessageId,
    status,
    errorCode: values.errorCode ? String(values.errorCode) : null,
    errorMessage: values.errorMessage ? String(values.errorMessage) : null,
...
async function parseGenericInbound(request: Request): Promise<InboundSmsEvent> {
  ...
  return {
    providerMessageId: String(
      values.messageId ?? values.id ?? values.MessageSid ?? "",
    ),
    from: String(values.from ?? values.source ?? values.From ?? ""),
    to: String(values.to ?? values.destination ?? values.To ?? ""),
    body: String(
      values.body ?? values.text ?? values.message ?? values.Body ?? "",
    ),
    payload: Object.fromEntries(
      Object.entries(values)
        .filter(([, value]) => typeof value === "string")
...
export function providerHasCredentials(provider: SmsProviderId) {
  switch (provider) {
    ...
    case "sinch":
      return Boolean(
        process.env.SINCH_SERVICE_PLAN_ID && process.env.SINCH_API_TOKEN,
      );
    case "infobip":
      return Boolean(
        process.env.INFOBIP_BASE_URL && process.env.INFOBIP_API_KEY,
      );
    case "bandwidth":
      return Boolean(
        process.env.BANDWIDTH_API_TOKEN &&
        process.env.BANDWIDTH_API_SECRET &&
        process.env.BANDWIDTH_APPLICATION_ID,
      );
    case "bird":
      return Boolean(
        process.env.BIRD_ACCESS_KEY &&
    ...
    case "azure_communication_services":
