...
async function requireLocationPermission(
  ...
  if (
    !(await hasLocationPermission(transaction, actorId, locationId, permission))
  )
    throw new DomainError("FORBIDDEN");
}
...
export async function updateOwnProfile(actorId: string, values: ProfileInput) {
  await db.transaction(async (transaction) => {
    await transaction
      ...
      .where(eq(userProfiles.id, actorId));
    await transaction
      .update(user)
...
export async function createCustomRole(
  ...
  return db.transaction(async (transaction) => {
    ...
    if (
      ...
      input.permissionKeys.length > 200
    )
    ...
    const permissionRows = input.permissionKeys.length
      ? await transaction
          ...
          .from(permissions)
          .where(sql`key = any(${input.permissionKeys}::text[])`)
      : [];
    ...
    await transaction.insert(auditLogs).values({
      organizationId,
      actorUserId: actorId,
      actorMembershipId: await actorMembership(
        transaction,
        actorId,
        organizationId,
      ),
      ...
      const [location] = await transaction
        ...
        .where(
          and(
            ...
            ne(pantryLocations.status, "archived"),
          ),
        )
        .limit(1);
      if (
...
export async function createLocation(
  actorId: string,
  organizationId: string,
  values: LocationInput & { slug: string },
  requestId: string,
) {
  return db.transaction(async (transaction) => {
    await requireOrganizationPermission(
    ...
    const [created] = await transaction
      ...
      .values({
        ...
        postalCode: values.postalCode || null,
        countryCode: values.countryCode,
        operatingNotes: values.operatingNotes || null,
        createdBy: actorId,
      })
      .returning();
    await transaction.insert(auditLogs).values({
...
export async function updateLocation(
  actorId: string,
  locationId: string,
  values: Omit<LocationInput, "slug">,
  requestId: string,
) {
  return db.transaction(async (transaction) => {
...
export async function archiveLocation(
  ...
  await db.transaction(async (transaction) => {
    ...
    const [{ value: activeCount }] = await transaction
      ...
      .where(
        and(
          eq(pantryLocations.organizationId, location.organizationId),
          ne(pantryLocations.status, "archived"),
        ),
      );
    ...
    await transaction
      ...
      .where(
        and(
          eq(membershipRoles.locationId, locationId),
          isNull(membershipRoles.archivedAt),
        ),
      );
    await transaction
      ...
      .where(
        and(
          ...
          ne(locationMemberships.status, "archived"),
        ),
      );
    await transaction
      .update(pantryLocations)
    ...
    await transaction
      ...
      .where(eq(userProfiles.defaultLocationId, locationId));
    await transaction.insert(auditLogs).values({
      ...
      requestId,
      reason,
      previousValues: { status: location.status },
...
export async function prepareInvitation(
  ...
  return db.transaction(async (transaction) => {
    ...
    const [role] = await transaction
      .select()
      .from(roles)
      .where(and(eq(roles.id, roleId), isNull(roles.archivedAt)))
      .limit(1);
    if (
      !role ||
      ...
      throw new DomainError("INVALID_ROLE_SCOPE");
    if (locationId) {
      const [location] = await transaction
    ...
    if (prior)
      await transaction
        ...
        .where(eq(organizationInvitations.id, prior.id));
...
export async function assignMemberLocation(
  actorId: string,
  organizationId: string,
  membershipId: string,
  locationId: string,
  ...
  await db.transaction(async (transaction) => {
    ...
    await transaction
      ...
      .onConflictDoUpdate({
        target: [
          locationMemberships.organizationMembershipId,
          locationMemberships.locationId,
        ],
    ...
    await transaction.insert(auditLogs).values({
      ...
      action: "location_membership.assigned",
...
export async function removeMemberLocation(
  actorId: string,
  organizationId: string,
  membershipId: string,
  locationId: string,
  ...
  await db.transaction(async (transaction) => {
    ...
    const [removed] = await transaction
      ...
      .where(
        and(
          eq(locationMemberships.organizationMembershipId, membershipId),
          eq(locationMemberships.organizationId, organizationId),
          eq(locationMemberships.locationId, locationId),
          ne(locationMemberships.status, "archived"),
        ),
      )
    ...
    if (target)
      await transaction
        ...
        .where(
          and(
            eq(userProfiles.id, target.userId),
            eq(userProfiles.defaultLocationId, locationId),
          ),
        );
    await transaction.insert(auditLogs).values({
      organizationId,
      ...
      action: "location_membership.removed",
      entityType: "organization_membership",
      entityId: membershipId,
      requestId,
      reason,
      previousValues: { locationId },
    });
  });
}
...
export async function assignMemberRole(
  ...
  return db.transaction(async (transaction) => {
    ...
    if (
      ...
      throw new DomainError("INVALID_ROLE_ASSIGNMENT");
    ...
    if (roleId === administratorRoleId && expiresAt)
      throw new DomainError("ADMINISTRATOR_CANNOT_EXPIRE");
    if (locationId) {
      const [assignment] = await transaction
        ...
        .where(
          and(
            ...
            isNull(locationMemberships.archivedAt),
          ),
        )
        .limit(1);
...
export async function removeMemberRole(
  ...
  await db.transaction(async (transaction) => {
    ...
    if (assignment.roleId === administratorRoleId)
      await requireOrganizationPermission(
        transaction,
        actorId,
        organizationId,
...
export async function changeMembershipStatus(
  ...
  await db.transaction(async (transaction) => {
    ...
    if (membership.status === "archived")
      throw new DomainError("MEMBERSHIP_BLOCKED");
    await transaction
    ...
    if (status === "active")
      await transaction
        ...
        .where(
          and(
            eq(locationMemberships.organizationMembershipId, membershipId),
            eq(locationMemberships.status, "suspended"),
            isNull(locationMemberships.archivedAt),
          ),
        );
    if (status === "archived") {
      ...
      await transaction
        .update(userProfiles)
        .set({ defaultOrganizationId: null, defaultLocationId: null })
        .where(
          and(
            eq(userProfiles.id, membership.userId),
            eq(userProfiles.defaultOrganizationId, organizationId),
          ),
        );
    }
    await transaction.insert(auditLogs).values({
...
export async function acceptInvitation(
  ...
  return db.transaction(async (transaction) => {
    ...
    if (
      !invitation ||
      invitation.email.toLowerCase() !== actorEmail.toLowerCase()
    )
      throw new DomainError("INVALID_OR_EXPIRED_INVITATION");
    const [role] = await transaction
      .select()
      .from(roles)
      .where(and(eq(roles.id, invitation.roleId), isNull(roles.archivedAt)))
      .limit(1);
    ...
    if (!membership) {
      [membership] = await transaction
        ...
        .values({
          ...
          userId: actorId,
          status: "active",
          allLocations: role.scope === "organization",
      ...
      [membership] = await transaction
        ...
        .returning();
    } else if (["suspended", "archived"].includes(membership.status))
      throw new DomainError("MEMBERSHIP_BLOCKED");
    ...
    const [existingRole] = await transaction
      ...
      .where(
        and(
          eq(membershipRoles.organizationMembershipId, membership.id),
          eq(membershipRoles.roleId, invitation.roleId),
          invitation.locationId
            ? eq(membershipRoles.locationId, invitation.locationId)
            : isNull(membershipRoles.locationId),
          isNull(membershipRoles.archivedAt),
          or(
            isNull(membershipRoles.expiresAt),
            gt(membershipRoles.expiresAt, new Date()),
          ),
        ),
      )
      .limit(1);
    ...
    await transaction.insert(auditLogs).values({
      ...
      requestId,
      previousValues: { status: "pending" },
      newValues: { status: "accepted", membershipId: membership.id },
    });
